...
userSchema.pre('save', async function(next) {
  // Only run this function here if password is modified
  if (!this.isModified('password')) return next();

  // Hash the password by bcrypt algorithm (Salting pass before hasing, salting means add random string to pass )
  this.password = await bcrypt.hash(this.password, 12); // 12 is cost encrytion, higher means CPU cost more time to encrypt pass
...
// Instance Method, which is available on all documments in certain collection
